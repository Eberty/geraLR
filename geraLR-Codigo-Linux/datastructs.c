 ...
 char                *moduleName )
{
 ...
 if (list == NULL) {
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Unable to create list of %u variable parameters (malloc failed)", totPos);
   data_error (0, moduleName, procName, auxErrorStr);
   return (NULL);
 }
...
static void check_DATA_set_compatibility (DATA_t_set_code set1, DATA_t_set_code set2, char *setOperation)
{
 ...
 if (p_GA_info1->patternSubsets != p_GA_info2->patternSubsets) {
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "%s of sets %d and %d failed: number of data pattern subsets differ (%u/%u)",
 ...
 for (iPatternSubset = 0; iPatternSubset < p_GA_info1->patternSubsets; iPatternSubset++) {
   ...
   if (patternSubsetStart1 != patternSubsetStart2) {
     snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "%s of sets %d and %d failed: start of pattern subset %u differ (%d/%d)",
   ...
   if (patternSubsetSize1 != patternSubsetSize2) {
     snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "%s of sets %d and %d failed: size of pattern subset %u differ (%d/%d)",
 ...
 int                  *dataItemPosition )
{
 ...
 if (arrayType == t_dataStructure_array) {
   ...
   if ((p_GA_info->totDataItems != 0) && (! p_GA_info->b_extensible)) {
     snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Invalid position %d in fixed size array %d: generic array overflow (max %u data items)", *dataItemPosition, arrayCode, absMaxDataItems);
     data_error (0, __FILE__, __func__, auxErrorStr);
     return;
   }
   ...
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Invalid position %d in array %d: generic array overflow (max %u data items)", *dataItemPosition, arrayCode, absMaxDataItems);
   data_error (0, __FILE__, __func__, auxErrorStr);
   return;
 }
/*
 ...
 *dataItemPosition = (int) p_GA_info->totDataItems + 1;
 if ((p_GA_info->absMaxDataItems > 0) && (*dataItemPosition > (int) p_GA_info->absMaxDataItems)) {
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Invalid position %d in %s %d: generic array overflow (max %u data items)", *dataItemPosition, dataType2dataTypeStr (arrayType), arrayCode, maxDataItems);
   data_error (0, __FILE__, __func__, auxErrorStr);
   return;
 }
}
 ...
 unsigned int         resizeFactor )
{
 ...
 if ((totZeros == 1) || (totZeros == 2)) {
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Invalid resizing parameters in %s %d: use either 3 or no zeros, not %u", dataType2dataTypeStr (dataType), arrayCode, totZeros);
   data_error (0, __FILE__, __func__, auxErrorStr);
 ...
 int          *p_patternSubsetParams )
{
 ...
 if ((patternSubsets > 0) && (p_patternSubsetParams == NULL)) {
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Expected list of data pattern parameters as patternSubsets > 0 (%u)", patternSubsets);
   data_error (0, __FILE__, __func__, auxErrorStr);
   return ((bool) false);
 }
 b_match = false;
 ...
 int          *p_patternSubsetParams )
{
 ...
 if ((patternSubsets > 0) && (p_dataPattern == NULL)) {
   snprintf (auxErrorStr, MAX_ERROR_MESSAGE_LENGTH, "Expected a data pattern as patternSubsets > 0 (%u)", patternSubsets);
   data_error (0, __FILE__, __func__, auxErrorStr);
   return ((DATA_t_array_code) UNKNOWN_GENERIC_ARRAY_CODE);
 }
