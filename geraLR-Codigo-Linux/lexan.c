   ...
   if (ptr == NULL) {
     ...
     if (! feof (p_jobInfo->filePt)) {
       snprintf (
         p_jobInfo->lastError,
		 ERROR_maxErrorMsgSize,
         "%s::read_char(%d): Read error. Forcing EOF in \"%s\" ",
         __FILE__,
         jobId,
         p_jobInfo->fileName );
       p_jobInfo->totErrors++;
 ...
 LEXAN_t_tokenSpecs *tokenSpecs )
{
 ...
 if (filePt == NULL) {
   snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize, "Cannot open file \"%s\" for reading", fileName);
 ...
 switch (tokenCase) {
   case (LEXAN_t_upperCase) :
     p_jobInfo->convProc = toupper;
     break;
   case (LEXAN_t_lowerCase) :
     p_jobInfo->convProc = tolower;
     break;
   default :
     p_jobInfo->convProc = NULL;
     break;
 }
 ...
 for (c_token = 0,
   ...
   if (p_tokenSpecs->type == LEXAN_token_OPEN_COMMENT) {
     if (b_openCOMMENT_used)
       ERROR_fatal_error (0, __FILE__, __func__,
         "LEXAN_token_OPEN_COMMENT can only be used once" );
   ...
   if (p_tokenSpecs->type == LEXAN_token_CLOSE_COMMENT) {
     if (b_closeCOMMENT_used)
       ERROR_fatal_error (0, __FILE__, __func__,
         "LEXAN_token_CLOSE_COMMENT can only be used once" );
   ...
   if (p_tokenInfo->tk_regexStr == NULL) {
     snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize, "tokenInfo[%d].tk_regexStr=strdup(\"%s\") failed",
   ...
   p_tokenInfo->tk_regexBuffer.allocated = (unsigned long)     0;
   ...
   p_tokenInfo->tk_regexBuffer.translate = (char *)            NULL;
   ...
   if (compResult) {
     snprintf (
       ERROR_auxErrorMsg,
	   ERROR_maxErrorMsgSize,
       "Regex compilation failed (job #%d, token #%d): %s",
       activeJobs - 1,
       c_token,
       compResult );
 ...
 if (b_openCOMMENT_used && (! b_closeCOMMENT_used))
   ERROR_fatal_error (0, __FILE__, __func__, "LEXAN_token_CLOSE_COMMENT missing");

 if (b_closeCOMMENT_used && (! b_openCOMMENT_used))
   ERROR_fatal_error (0, __FILE__, __func__, "LEXAN_token_OPEN_COMMENT missing");
 ...
 if (p_jobInfo->fileName == NULL) {
   snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize, "jobInfo[%d].fileName = strdup (\"%s\") failed",
     activeJobs - 1,
     fileName );
   ERROR_fatal_error (errno, __FILE__, __func__, ERROR_auxErrorMsg);
 }
 ...
 if (p_jobInfo->b_midComment) {
   ...
   if (p_jobInfo->currChar == c_EOF) {
     ...
     snprintf (
       p_jobInfo->lastError,
	   ERROR_maxErrorMsgSize,
       "%s(): Comment opened in line %d of \"%s\" was not closed",
       __func__,
   ...
   if (c_token == p_jobInfo->formalTokens)
     ERROR_fatal_error (
       ...
       __FILE__,
       __func__,
       "Panic! Cannot find LEXAN_token_CLOSE_COMMENT");
   ...
   switch (matchSize) {
     ...
     case (-2):
       ...
       snprintf (
         p_jobInfo->lastError,
		 ERROR_maxErrorMsgSize,
         "%s(): Internal error in re_search(). Skipping line %d of \"%s\"",
         __func__,
         p_jobInfo->openCommLineNum,
         p_jobInfo->fileName );
     ...
     case (-1):
       ...
       if (! b_consumedOK) {
         ...
         snprintf (
           p_jobInfo->lastError,
		   ERROR_maxErrorMsgSize,
           "%s(): Could not skip comments in \"%s\"\n[%4d] %s%s",
           __func__,
           p_jobInfo->fileName,
           p_jobInfo->lineNum,
           p_jobInfo->currLine,
     ...
     default:
       ...
       if (! b_consumedOK) {
         ...
         snprintf (
           p_jobInfo->lastError,
		   ERROR_maxErrorMsgSize,
           "%s(): Could not skip end-of-comment in \"%s\"\n[%4d] %s%s",
           __func__,
           p_jobInfo->fileName,
           p_jobInfo->lineNum,
           p_jobInfo->currLine,
 ...
 if ( (p_tokenInfo->tk_type == LEXAN_token_OPEN_COMMENT) &&
   ...
   if (! b_consumedOK) {
     ...
     snprintf (
       p_jobInfo->lastError,
	   ERROR_maxErrorMsgSize,
       "%s(): Could not skip beginning-of-comment in \"%s\"\n[%4d] %s%s",
       __func__,
       p_jobInfo->fileName,
       p_jobInfo->lineNum,
       p_jobInfo->currLine,
 ...
 if (p_tokenInfo->tk_type == LEXAN_token_LINE_COMMENT) {
   ...
   if (! b_consumedOK) {
     ...
     snprintf (
       p_jobInfo->lastError,
	   ERROR_maxErrorMsgSize,
       "%s(): Could not skip one-line comment in \"%s\"\n[%4d] %s%s",
       __func__,
       p_jobInfo->fileName,
       p_jobInfo->lineNum,
       p_jobInfo->currLine,
 ...
 switch (p_tokenInfo->tk_type) {
   ...
   case (LEXAN_token_double_str) :
     ...
     if (p_jobInfo->tokenVal.tokenStr == NULL)
       ERROR_no_memory (errno, __FILE__, __func__,
       "p_jobInfo->tokenVal.tokenStr");
  ...
  LEXAN_t_tokenVal *p_subTokenVal )
{
 ...
 if (subGroupStart > subGroupEnd) {
   snprintf (
     p_jobInfo->lastError,
	 ERROR_maxErrorMsgSize,
     "%s(): Groups out of order (%d,%d) in \"%s\"\n[%4d] %s%s",
     __func__,
     subGroupStart,
     subGroupEnd,
     p_jobInfo->fileName,
 ...
 if (subGroupEnd > subExpr) {
   snprintf (
     p_jobInfo->lastError,
	 ERROR_maxErrorMsgSize,
     "%s(): Invalid group range (max %d) in \"%s\"\n[%4d] %s%s",
     __func__,
     subExpr,
     p_jobInfo->fileName,
     p_jobInfo->lineNum,
...
char *LEXAN_get_token_regex (LEXAN_t_lexJobId jobId)
{
 ...
 if (tokenRegex == NULL) {
   snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize, "tokenRegex = strdup (\"%s\") failed",
     p_tokenInfo->tk_regexStr );
   ERROR_fatal_error (errno, __FILE__, __func__, ERROR_auxErrorMsg);
 }
...
/*
*---------------------------------------------------------------------
*  Return pointer to input line where the current token occurred
*---------------------------------------------------------------------
*/
...
*/
 ...
 errno = 0;
 strcpy (globalCurrLine, p_jobInfo->currLine);
...
bool LEXAN_terminate_job (LEXAN_t_lexJobId jobId)
{
 ...
 free ((char *) tokenStr);
 tokenStr = (char *) NULL;
 free ((char *) p_jobInfo->fileName);
 free ((char *) p_jobInfo->currLine);
 free ((char *) p_jobInfo->lastError);
 free ((char *) p_jobInfo->tokenVal.tokenStr);

 for (c_token = 0, p_tokenInfo = p_jobInfo->tokenInfoVec;
        c_token < p_jobInfo->formalTokens;
          c_token++, p_tokenInfo++ ) {
   free ((char *) p_tokenInfo->tk_regexStr);
   free ((char *) p_tokenInfo->tk_regexRegs.start);
   free ((char *) p_tokenInfo->tk_regexRegs.end);
   regfree ((regex_t *) &(p_tokenInfo->tk_regexBuffer));
 }
 free ((t_tokenInfo *) p_jobInfo->tokenInfoVec);
 ...
 if (kill_this < activeJobs-1)
   memcpy (
     &(jobInfoVec[kill_this]),
     &(jobInfoVec[activeJobs-1]),
     sizeof (t_jobInfo) );
