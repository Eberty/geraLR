...
*   File         : mystrings.c
*   Created      : 2012-05-30
*   Last Modified: 2012-05-31 (2018-03-30 STRING_clear() warning removed)
*
...
char *STRING_clear (char **stringVar) {
  ...
  *stringVar = (char *) realloc ((void *) *stringVar, sizeof(char));
  if (*stringVar == NULL) {
    //snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize, "STRING_clear(\"%s\"): realloc failed", *stringVar);
    snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize, "STRING_clear(char **stringVar): realloc failed: (*stringVar == NULL)");
...
char *STRING_set (char **stringVar, unsigned char charToSet, size_t timesToAdd) {
  ...
  if (resultString == NULL) {
    snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize,
      "STRING_set(\"%s\",'%c',%zd): realloc(\"%s\",%d) failed",
      *stringVar, charToSet, timesToAdd, *stringVar, (int) resultStringMemBlockSize);
    ERROR_fatal_error (errno, __FILE__, __func__, ERROR_auxErrorMsg);
  }
...
char *STRING_extend (char **stringVar, unsigned char charToAdd, size_t timesToAdd) {
  ...
  if (resultString == NULL) {
    snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize,
      "STRING_extend(\"%s\",'%c',%zd): realloc(\"%s\",%d) failed",
      *stringVar, charToAdd, timesToAdd, *stringVar, (int) resultStringMemBlockSize);
    ERROR_fatal_error (errno, __FILE__, __func__, ERROR_auxErrorMsg);
  }
...
*/
 ...
 if (resultString == NULL) {
   snprintf (ERROR_auxErrorMsg, ERROR_maxErrorMsgSize,
     "STRING_justify(\"%s\",%zd,'%c',%d): malloc(%d) failed",
     *originalString, finalLength, filler, (int) justification, (int) resultStringMemBlockSize);
   ERROR_fatal_error (errno, __FILE__, __func__, ERROR_auxErrorMsg);
 }
