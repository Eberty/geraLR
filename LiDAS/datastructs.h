...
All data structures provided in this module are dinamically allocated and can
grow as needed during program execution. How they grow, or whether they are 
allowed to grow at all, is determined by their creation paramaters.
...
the SET creation methods take extra arguments, described shortly.
 ...
 (3) initialBlockSize: the data structure is initialized with just enough space
     ...
     a dinamically allocated data structure, e.g. a set element). If at any 
...
only way to access the corresponing data structure.
...
Anther method takes variable length argument lists: DATA_multiple_store_in_array().
